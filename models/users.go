...
var (
	userAllColumns            = []string{"id", "username", "email", "password_hash"}
...
func (o *User) Channels(mods ...qm.QueryMod) channelQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.InnerJoin("\"channel_users\" on \"channels\".\"id\" = \"channel_users\".\"channel_id\""),
		qm.Where("\"channel_users\".\"user_id\"=?", o.ID),
	)
...
func (o *User) HubPermissions(mods ...qm.QueryMod) hubPermissionQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"hub_permissions\".\"user_id\"=?", o.ID),
	)
...
func (o *User) Hubs(mods ...qm.QueryMod) hubQuery {
	...
	queryMods = append(queryMods,
		qm.InnerJoin("\"hub_users\" on \"hubs\".\"id\" = \"hub_users\".\"hub_id\""),
		qm.Where("\"hub_users\".\"user_id\"=?", o.ID),
	)
...
// CreatorHubs retrieves all the hub's Hubs with an executor via creator column.
func (o *User) CreatorHubs(mods ...qm.QueryMod) hubQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadChannels(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadHubPermissions(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		...
		qm.WhereIn(`hub_permissions.user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (userL) LoadHubs(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.Select("\"hubs\".id, \"hubs\".name, \"hubs\".creator, \"a\".\"user_id\""),
		qm.From("\"hubs\""),
		qm.InnerJoin("\"hub_users\" as \"a\" on \"hubs\".\"id\" = \"a\".\"hub_id\""),
		qm.WhereIn("\"a\".\"user_id\" in ?", args...),
	)
	if mods != nil {
		mods.Apply(query)
	...
	for results.Next() {
		...
		err = results.Scan(&one.ID, &one.Name, &one.Creator, &localJoinCol)
		if err != nil {
			return errors.Wrap(err, "failed to scan eager loaded results for hubs")
		}
		if err = results.Err(); err != nil {
			return errors.Wrap(err, "failed to plebian-bind eager loaded slice hubs")
		}
...
// LoadCreatorHubs allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadCreatorHubs(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`hubs`),
		qm.WhereIn(`hubs.creator in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.CreatorHubs = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &hubR{}
			}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if queries.Equal(local.ID, foreign.Creator) {
				local.R.CreatorHubs = append(local.R.CreatorHubs, foreign)
				if foreign.R == nil {
					foreign.R = &hubR{}
				}
...
func (o *User) AddChannels(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Channel) error {
	var err error
	for _, rel := range related {
		if insert {
			if err = rel.Insert(ctx, exec, boil.Infer()); err != nil {
	...
	for _, rel := range related {
		query := "insert into \"channel_users\" (\"user_id\", \"channel_id\") values ($1, $2)"
...
// Sets o.R.Users's Channels accordingly.
// Replaces o.R.Channels with related.
// Sets related.R.Users's Channels accordingly.
func (o *User) SetChannels(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Channel) error {
	query := "delete from \"channel_users\" where \"user_id\" = $1"
	...
	removeChannelsFromUsersSlice(o, related)
	if o.R != nil {
		o.R.Channels = nil
	}
...
func (o *User) RemoveChannels(ctx context.Context, exec boil.ContextExecutor, related ...*Channel) error {
	if len(related) == 0 {
		return nil
	}
	...
	var err error
	query := fmt.Sprintf(
		"delete from \"channel_users\" where \"user_id\" = $1 and \"channel_id\" in (%s)",
...
func removeChannelsFromUsersSlice(o *User, related []*Channel) {
	for _, rel := range related {
		if rel.R == nil {
			continue
		}
...
// Sets related.R.User appropriately.
func (o *User) AddHubPermissions(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*HubPermission) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.UserID = o.ID
...
func (o *User) AddHubs(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Hub) error {
	...
	for _, rel := range related {
		query := "insert into \"hub_users\" (\"user_id\", \"hub_id\") values ($1, $2)"
		values := []interface{}{o.ID, rel.ID}
...
// SetHubs removes all previously related items of the
// user replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Users's Hubs accordingly.
// Replaces o.R.Hubs with related.
// Sets related.R.Users's Hubs accordingly.
func (o *User) SetHubs(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Hub) error {
	query := "delete from \"hub_users\" where \"user_id\" = $1"
	values := []interface{}{o.ID}
	if boil.IsDebug(ctx) {
		writer := boil.DebugWriterFrom(ctx)
		fmt.Fprintln(writer, query)
	...
	removeHubsFromUsersSlice(o, related)
	if o.R != nil {
		o.R.Hubs = nil
	}
	return o.AddHubs(ctx, exec, insert, related...)
}

// RemoveHubs relationships from objects passed in.
// Removes related items from R.Hubs (uses pointer comparison, removal does not keep order)
// Sets related.R.Users.
func (o *User) RemoveHubs(ctx context.Context, exec boil.ContextExecutor, related ...*Hub) error {
	if len(related) == 0 {
		return nil
	}
	...
	var err error
	query := fmt.Sprintf(
		"delete from \"hub_users\" where \"user_id\" = $1 and \"hub_id\" in (%s)",
		strmangle.Placeholders(dialect.UseIndexPlaceholders, len(related), 2, 1),
	)
	values := []interface{}{o.ID}
	for _, rel := range related {
	...
	for _, rel := range related {
		for i, ri := range o.R.Hubs {
			...
			ln := len(o.R.Hubs)
			if ln > 1 && i < ln-1 {
				o.R.Hubs[i] = o.R.Hubs[ln-1]
			}
			o.R.Hubs = o.R.Hubs[:ln-1]
			break
		}
	}
...
func removeHubsFromUsersSlice(o *User, related []*Hub) {
	for _, rel := range related {
		if rel.R == nil {
			continue
		}
...
// AddCreatorHubs adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.CreatorHubs.
// Sets related.R.CreatorUser appropriately.
func (o *User) AddCreatorHubs(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Hub) error {
	var err error
	for _, rel := range related {
		if insert {
	...
	if o.R == nil {
		...
		o.R.CreatorHubs = append(o.R.CreatorHubs, related...)
	}
...
// SetCreatorHubs removes all previously related items of the
// user replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.CreatorUser's CreatorHubs accordingly.
// Replaces o.R.CreatorHubs with related.
// Sets related.R.CreatorUser's CreatorHubs accordingly.
func (o *User) SetCreatorHubs(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Hub) error {
	query := "update \"hubs\" set \"creator\" = null where \"creator\" = $1"
	values := []interface{}{o.ID}
	if boil.IsDebug(ctx) {
		writer := boil.DebugWriterFrom(ctx)
		fmt.Fprintln(writer, query)
	...
	if o.R != nil {
		for _, rel := range o.R.CreatorHubs {
	...
	return o.AddCreatorHubs(ctx, exec, insert, related...)
}

// RemoveCreatorHubs relationships from objects passed in.
// Removes related items from R.CreatorHubs (uses pointer comparison, removal does not keep order)
// Sets related.R.CreatorUser.
func (o *User) RemoveCreatorHubs(ctx context.Context, exec boil.ContextExecutor, related ...*Hub) error {
	if len(related) == 0 {
		return nil
	}
	...
	for _, rel := range related {
		for i, ri := range o.R.CreatorHubs {
			if rel != ri {
				continue
			}

			ln := len(o.R.CreatorHubs)
			if ln > 1 && i < ln-1 {
