...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (hubL) LoadMessages(ctx context.Context, e boil.ContextExecutor, singular bool, maybeHub interface{}, mods queries.Applicator) error {
	var slice []*Hub
	var object *Hub
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if queries.Equal(local.ID, foreign.HubID) {
...
// Sets related.R.Hub appropriately.
func (o *Hub) AddMessages(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Message) error {
	var err error
	for _, rel := range related {
		if insert {
			queries.Assign(&rel.HubID, o.ID)
			if err = rel.Insert(ctx, exec, boil.Infer()); err != nil {
				return errors.Wrap(err, "failed to insert into foreign table")
			}
		} else {
...
// hub replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Hub's Messages accordingly.
// Replaces o.R.Messages with related.
// Sets related.R.Hub's Messages accordingly.
func (o *Hub) SetMessages(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Message) error {
	query := "update \"messages\" set \"hub_id\" = null where \"hub_id\" = $1"
	values := []interface{}{o.ID}
	if boil.IsDebug(ctx) {
		writer := boil.DebugWriterFrom(ctx)
		fmt.Fprintln(writer, query)
	...
	if o.R != nil {
		for _, rel := range o.R.Messages {
			queries.SetScanner(&rel.HubID, nil)
			if rel.R == nil {
				continue
			}
...
// Sets related.R.Hub.
func (o *Hub) RemoveMessages(ctx context.Context, exec boil.ContextExecutor, related ...*Message) error {
	if len(related) == 0 {
		return nil
	}
	...
	for _, rel := range related {
		...
		if _, err = rel.Update(ctx, exec, boil.Whitelist("hub_id")); err != nil {
			return err
		}
	}
	if o.R == nil {
